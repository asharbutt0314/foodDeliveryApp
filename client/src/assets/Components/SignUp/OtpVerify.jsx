...
const OtpVerify = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/Clientauth/signup-otp-verify', {
        email,
        otp,
      });
  ...
  const handleResendOtp = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/Clientauth/resend-otp', { email });
      if (res.data.success) {
        ...
        <div className="col-lg-5 col-md-7">
          <div className="card shadow-lg border-0" style={{ borderRadius: '20px', overflow: 'hidden' }}>
            <div className="card-header text-white text-center py-4" style={{ background: 'linear-gradient(135deg, #FF5722 0%, #FF9800 100%)' }}>
              ...
              <p className="mb-0 mt-2" style={{ opacity: '0.9' }}>Enter the OTP sent to your business email</p>
            </div>
            
            <div className="card-body p-4" style={{ background: 'linear-gradient(145deg, #ffffff, #f8f9fa)' }}>
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  <label className="form-label fw-bold" style={{ color: '#FF5722' }}><i className="bi bi-envelope"></i> Business Email</label>
                  <input
                    type="email"
                    name="email"
                ...
                <div className="mb-4">
                  <label className="form-label fw-bold" style={{ color: '#FF5722' }}><i className="bi bi-key"></i> Verification Code</label>
                  <input
                    type="text"
                    name="otp"
                    className="form-control"
                ...
                >
                  <i className="bi bi-check-circle"></i> Verify & Activate Account
                </button>
                ...
                >
                  <i className="bi bi-arrow-clockwise"></i> Resend Verification Code
                </button>
                ...
                <div className="text-center mt-3">
                  <p className="text-muted small">Check your business email for the verification code. Don't forget to check spam folder!</p>
                </div>
              </form>
              
              <div className="text-center mt-4 pt-3" style={{ borderTop: '1px solid #FFE0B2' }}>
                <p className="text-muted mb-2">Need to access your restaurant account?</p>
                <div className="d-flex justify-content-center gap-3">
                  ...
                  >
                    <i className="bi bi-building"></i> Register Restaurant
                  </button>
                </div>
              </div>
            </div>
